...
kotlin {
    sourceSets {
        commonMain.dependencies {
            implementation("eu.tilomaps:tilo-compose:0.1.3-alpha09")
            // Optional drawing plugin:
            implementation("eu.tilomaps:tilo-compose-draw:0.1.3-alpha09")
        }
    }
}
```
...
| Loading and errors | ✅ Done | Raster declarations expose initialization plus available/degraded/offline/empty state, structured per-layer fetch/decode diagnostics, explicit retry, and bounded failure backoff.                                                                                     |
| Testing strategy | ✅ Done | CI runs unit tests, quality gates, Android emulator pixel contracts, iOS simulator suites, JVM tests for platform-independent modules, and publication verification against a coordinate-only consumer.                                                               |
| Documentation | ✅ Done | Versioned Learn and curated API reference pages are published; public common API KDoc is enforced, and CI generates and link-checks one Dokka publication across all six artifacts.                                                                                |
...
| Event routing | ⬜ Planned | Define gesture priority between overlays, selection, and default pan/zoom/rotate interactions.                                                                                                                                                                        |
