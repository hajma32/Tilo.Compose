 ...
 */
@ExperimentalTiloApi
fun argb(value: Long): ColorValue = ColorValue((value and 0xFFFFFFFFL).toULong())

/**
 * Builds style for point geometries.
 */
@ExperimentalTiloApi
fun pointStyle(block: PointStyleBuilder.() -> Unit = {}): PointStyle = PointStyleBuilder().apply(block).build()

/**
 * Builds style for line geometries.
 */
@ExperimentalTiloApi
fun lineStyle(block: LineStyleBuilder.() -> Unit = {}): LineStyle = LineStyleBuilder().apply(block).build()

/**
 * Builds style for polygon geometries.
 */
@ExperimentalTiloApi
fun polygonStyle(block: PolygonStyleBuilder.() -> Unit = {}): PolygonStyle = PolygonStyleBuilder().apply(block).build()
...
class FeatureLayerStyleBuilder {
    ...
    fun zoom(
        ...
        block: FeatureLayerStyleZoomRuleBuilder.() -> Unit,
    ) {
        zoomRules +=
            FeatureLayerStyleZoomRuleBuilder(minZoom, maxZoomExclusive)
                .apply(block)
                .build()
    }
    ...
    internal fun build(): FeatureLayerStyle =
        FeatureLayerStyle(
            ...
            zoomRules = zoomRules.toList(),
        )
}

/** Builder for one zoom-dependent partial [FeatureLayerStyle] replacement. */
@ExperimentalTiloApi
@TiloDsl
class FeatureLayerStyleZoomRuleBuilder internal constructor(
    private val minZoom: Double?,
    private val maxZoomExclusive: Double?,
) {
    private var point: PointStyle? = null
    private var line: LineStyle? = null
    private var polygon: PolygonStyle? = null
    ...
    internal fun build(): FeatureLayerStyleZoomRule =
        FeatureLayerStyleZoomRule(
            minZoom = minZoom,
            maxZoomExclusive = maxZoomExclusive,
            point = point,
            line = line,
            polygon = polygon,
            label = label,
...
class LabelStyleBuilder internal constructor(
    ...
    var textAlign: LabelTextAlign = LabelTextAlign.Center
...
class LineStyleBuilder {
    ...
    internal fun build(): LineStyle = LineStyle(casing = casing, stroke = stroke)
}
...
class PolygonStyleBuilder {
    ...
    internal fun build(): PolygonStyle = PolygonStyle(fill = fill, casing = casing, stroke = stroke)
}
...
class FillStyleBuilder internal constructor(
    ...
    internal fun build(): FillStyle = FillStyle(color = color, opacity = opacity, pattern = pattern)
}

/** Options for the additional-width casing drawn below a foreground stroke. */
@ExperimentalTiloApi
@TiloDsl
class CasingStyleBuilder internal constructor(
    private val color: ColorValue,
